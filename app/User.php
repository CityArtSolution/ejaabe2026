...
{
    use Notifiable;
    //to get users of  selected branch on the admin panel
     ...
     public static function boot()
{
    ...
    self::creating(function ($model) {
        // إذا ما تم تمرير branch_id، خذ القيمة من session أو 1
        $model->branch_id = $model->branch_id ?? session()->get('admin_selected_branch') ?? 1;
    });
 ...
 public function branch()
    ...
    //check if  user has exams to redirect it  to specefic  page

public function hasExamWebinars()
{
    ...
    $examWebinars =Webinar::whereHas('sales', function ($query) {
        $query->where('buyer_id', $this->id) // Filter by the user's ID
              ->whereNull('refund_at'); // Exclude refunded sales
    })->where('type', 'exam') // Filter webinars by type 'exam'
      ->exists();
    // Check if any of the webinars in these sales are of type 'exam'
         if ($examWebinars) {
            return true;
            }
...
//to use it  on sidebar and  details  of webinars  page  on user panel
...
public function isCanadaBranch()
{
    return $this->email === 'admin.ca@ejaabi.com';
}
