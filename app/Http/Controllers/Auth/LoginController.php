...
//        dd($validate->errors());
        if ($validate->fails()) {
            ...
            return back()->withErrors($validate->errors())->withInput($request->all());
    ...
    public function logout(Request $request)
    {
        ...
        if (!empty($user) && $user->logged_count > 0) {
            $user->update([
        ...
        // الشرط على branch_id
        if (!empty($user) && $user->branch_id == 3) {
...
//        ]);
        ...
        return redirect()->back()
            ->withErrors($errors)
            ->withInput($request->only($this->getUsername($request)));
    }
    ...
    public function afterLogged(Request $request, $verify = false)
    {
        ...
        if ($user->isAdmin()) {
            ...
            //check if  user  has  exams  and not a teacher redirect it  to  display  exams page
            if ($user->hasExamWebinars() && !$user->isTeacher()) {
