   ...
   public function charge(Request $request)
{
    ...
    if ($orderId) {
        ...
        if (!$order) {
            return back()->with('error', 'No pending order found with this ID');
        }
    }
    ...
    $rules = [
        ...
        'gateway' => 'required',
        'bank_name' => 'required_if:gateway,offline|string|max:255',
        'transfer_name' => 'required_if:gateway,offline|string|max:255',
        'receipt_number' => 'required_if:gateway,offline|string|max:255',
        'date' => 'required_if:gateway,offline|date',
    ];
    ...
    if ($gateway === 'offline') {
        ...
        if ($order) {
            ...
            foreach ($order->orderItems as $orderItem) {
                \App\Models\Sale::createSales($orderItem, $order->payment_method);
            }

            \App\Models\Cart::emptyCart($order->user_id);
        }
