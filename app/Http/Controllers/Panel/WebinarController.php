...
class WebinarController extends Controller
{
    ...
    if (!$activePackage->days_remained) {
             return redirect()->back()->with(['toast' => [
                    'msg' =>'لاتوجد لديك باقة فعاله ,قم بالاشتراك لادارة الدورات التدريبية',
                    'status' => 'error'
                ]]);
        }
    ...
    public function purchases(Request $request)
    {
        ...
        $query = Sale::query()
            ...
            //check if  the  user  has exam then we  get only exams
            $hasExamWebinars = $user->hasExamWebinars(); 

        $sales = deepClone($query)
            ->with([
                'webinar' => function ($query)use($hasExamWebinars) {
                    $query->with([
                        'files',
                        'reviews' => function ($query) {
                            $query->where('status', 'active');
