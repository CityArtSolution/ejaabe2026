...
$webinarTitle = $quiz->title; // fallback in case not found in translation
        ...
        if (!empty($quiz->webinar_id)) {
            ...
            if ($webinarTranslation && !empty($webinarTranslation->title)) {
...
// return $webinarTitle;
 ...
 public function printquizzes(Request $request, $id)
    {
        $quiz = Quiz::where('id', $id)->first();
        ...
        if ($quiz) {
            ...
            if (!isset($quiz->attempt) or ($userQuizDone->count() < $quiz->attempt and !$status_pass)) {
                ...
                return view(getTemplate() . '.panel.quizzes.print', $data);
            } else {
                $toastData = [
                    'title' => trans('public.request_failed'),
                    'msg' => trans('quiz.cant_start_quiz'),
  ...
  /*
  $totalCorrectAnswers = QuizzesQuestionsAnswer::where('question_id', $question->id)
  ->where('creator_id', $quiz->creator_id)
  ->where('correct', 1)
  ->count();

// Prepare to track student's correct answers
$studentCorrectAnswers = 0;
$results[$questionId]['status'] = false;
$results[$questionId]['grade'] = 0;

// Ensure $result['answer'] is always an array
$selectedAnswers = is_array($result['answer']) ? $result['answer'] : [$result['answer']];
...
if ($totalCorrectAnswers > 0) {
  ...
  $gradePerCorrectAnswer = $question->grade / $totalCorrectAnswers;
  $obtainedGrade = $studentCorrectAnswers * $gradePerCorrectAnswer;

  // Set results
  $results[$questionId]['grade'] = $obtainedGrade;
  $results[$questionId]['status'] = $studentCorrectAnswers > 0;

  // Add to total mark
  if ($studentCorrectAnswers > 0) {
      $totalMark += $obtainedGrade;
  }
}
*/
...
if ($totalCorrectAnswers > 1) {
    ...
    // For single correct answer scenario
    foreach ($selectedAnswers as $answerId) {
        $answer = QuizzesQuestionsAnswer::where('id', $answerId)
            ->where('question_id', $question->id)
            ->where('creator_id', $quiz->creator_id)
             ...
             $agent = $_SERVER['HTTP_USER_AGENT'];
              ...
              $browserInfo = $this->xapiService->getBrowserInfo($agent);
                ...
                $params = [
                    'name' => $user->full_name,
                    'email' => $user->email,
                    'course_url' => $quiz->webinar->getUrl(),
                    'course_nameAr' => $quiz->webinar->title,
                    'course_nameEn' =>$quiz->webinar->getTranslation('title','en')->title ?? $quiz->webinar->title,
                    'type' => 'https://w3id.org/xapi/cmi5/activitytype/course',
                    'browser' =>$browserInfo['browser'],
                    'version' =>$browserInfo['version'],
                    'platform' => 'EJAABI',
                    'instractor_name' =>$quiz->webinar->teacher->full_name,
                    'instractor_email' => $quiz->webinar->teacher->email,
                    'parent_url' => $quiz->webinar->getUrl(),
  ...
  //eval post
 public function quizzesStoreResultEval(Request $request, $id)
    {
        //$user = auth()->user();
        $quiz = Quiz::where('id', $id)->first();
...
if ($totalCorrectAnswers > 1) {
    ...
    foreach ($selectedAnswers as $answerId) {
        ...
        if ($answer && $answer->correct) {
            ...
            $totalMark += $question->grade;
                    ...
                    $results["info"]["name"]=$request->get('full_name');
    ...
    public function myResults(Request $request)
    {
        ...
        foreach ($quizResults->groupBy('quiz_id') as $quiz_id => $quizResult) {
            ...
            if (!isset($quiz->attempt) or (count($quizResult) < $quiz->attempt)) {
