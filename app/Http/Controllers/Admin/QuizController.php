...
use App\Exports\EvalResultsExport;
...
class QuizController extends Controller
{
    public function index(Request $request)
    {
        ...
        //$query = Quiz::query();
 $query = Quiz::query()->byBranch()->whereHas('webinar', function($q) {
            $q->where('type', '!=', 'exam');
            
        });
        $totalQuizzes = deepClone($query)->count();
        $totalActiveQuizzes = deepClone($query)->where('status', 'active')->count();
        $totalStudents = QuizzesResult::byBranch()->groupBy('user_id')->count();
        $totalPassedStudents = QuizzesResult::byBranch()->where('status', 'passed')->groupBy('user_id')->count();
    ...
    public function edit(Request $request, $id)
    {
        ...
        $webinars = Webinar::/*where('status', 'active')*/where(function ($query) use ($creator) {
    ...
    public function delete(Request $request, $id)
    {
     ...
     public function evalResults(Request $request,$id, $is_export_excel = false)
    {
    ...
    if ($request->has('action')) {
        ...
        if ($action === 'تصدير الى اكسل') {
            $export = new EvalResultsExport($quizzesResults);
            
            return Excel::download($export, 'quiz_eval_result.xlsx');
      ...
      $data = [
            ...
            return view('admin.reports.evalresults', $data);   
