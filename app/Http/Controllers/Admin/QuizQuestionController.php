...
class QuizQuestionController extends Controller
{
    public function store(Request $request)
    {
      ...
      //edit_eval
     public function editeval($question_id)
    {
        ...
        if (!empty($question)) {
            ...
            if (!empty($quiz)) {
                ...
                if ($question->type == 'multiple') {
                    $html = (string)\View::make('admin.quizzes.modals.multiple_question_eval', $data);
                } else {
...
//edit_cat_eval
     public function editcateval($question_id)
    {
        $question = EvalCategory::where('id', $question_id)->first();
        ...
        if (!empty($question)) {
            $quiz = Quiz::find($question->quiz_id);
                ...
                $data = [
                    ...
                    $html = (string)\View::make('admin.quizzes.modals.eval_category', $data);
    ...
    public function import(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'تم استيراد الاسئلة بنجاح!');
           ...
           /* return response()->json([
            ...
            return redirect()->back()->with('error','خطأ في استيراد الاسئلة');
                        ...
                        if (!empty($answer['title']) or !empty($answer['file'])) {
                            ...
                            if (!empty($quizQuestionsAnswer)) {
                                $quizQuestionsAnswer->update([
                                    'question_id' => $quizQuestion->id,
                                    'creator_id' => $creator->id ?? auth()->user()->id,
                                    'image' => $answer['file'] ?? null,
                                    'correct' => isset($answer['correct']) ? true : false,
                                    'created_at' => time()
                                ]);
