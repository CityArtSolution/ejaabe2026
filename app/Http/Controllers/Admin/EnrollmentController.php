...
class EnrollmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($user)) {
            ...
            $course=\App\Models\Webinar::find($data['webinar_id']);
            if($course){
              $user = User::find($data['user_id']);
  ...
  public function import_old(Request $request)
    {
        ...
        try {
            $import = new EnrollmentImport($request->webinar_ids);
            ...
            $toastData = [
                ...
                'msg' => "تم التسجيل بنجاح: {$results['success']}, Failed: {$results['failed']}",
...
//dd(  $results);
        $toastData = [
            'title' => 'Import Results',
            'msg' => "تم الانشاء: {$results['created']}, تحديث: {$results['updated']}, " .
                    "تم تسجيل: {$results['success']}, فشل: {$results['failed']}",
            'status' => ($results['failed'] == 0) ? 'success' : 'warning'
        ];
