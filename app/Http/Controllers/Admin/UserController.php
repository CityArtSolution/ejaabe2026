...
use App\Exports\ExamsResultsExport;
...
class UserController extends Controller
{
    ...
    public function staffs(Request $request)
    {
        ...
        $query = User::byBranch()->whereIn('role_id', $staffsRoleIds);
    ...
    public function organizations(Request $request, $is_export_excel = false)
    {
        ...
        $query = User::byBranch()->where('role_name', Role::$organization);
    ...
    public function getResults(Request $request, $is_export_excel = false)
    {
        $this->authorize('admin_quizzes_results');
     
        $query = User::byBranch()->where('role_name', Role::$user);
...
//        dd($query );
            $users = $query->orderBy('created_at', 'desc')->get();
           /* $quizzesResults = \App\Models\QuizzesResult::query()
            ->with(['quiz', 'user'])
            ...
            ->selectRaw("MAX(user_grade) as max_grade")
      ...
      /*      $quizzesResults = \App\Models\QuizzesResult::query()
    ->with(['quiz', 'user']) // Eager load quiz and user relationships
    ->when($request->student_id, function ($query, $studentId) {
        return $query->where('user_id', $studentId); // Filter by student_id
    })->where('status','!=','waiting')
    ->select('quizzes_results.*') // Select all columns from quizzes_results
    ->selectRaw('COUNT(*) as attempt_count') // Count attempts
    ->selectRaw('MAX(user_grade) as max_grade') // Get the maximum grade
    ->selectRaw('(SELECT id FROM quizzes_results AS sq 
                  WHERE sq.quiz_id = quizzes_results.quiz_id 
                    AND sq.user_id = quizzes_results.user_id 
                    AND sq.user_grade = MAX(quizzes_results.user_grade) 
                  LIMIT 1) AS max_grade_result_id') // Subquery to get the ID of the row with max grade
    ->groupBy('quiz_id', 'user_id') // Group by quiz_id and user_id
    ...
    $quizzesResults = \App\Models\QuizzesResult::query()
    ->with(['quiz', 'user' => function($query) {
        $query->with('userGroup.group'); // Eager load userGroup and its related group
    }])
    ->when($request->student_id, function ($query, $studentId) {
        return $query->where('user_id', $studentId);
    })
    ->when($request->group_id, function ($query, $groupId) {
        return $query->whereHas('user.userGroup', function ($q) use ($groupId) {
            $q->where('group_id', $groupId);
        });
    })
    ->where('status', '!=', 'waiting')
    ->select('quizzes_results.*')
    ->selectRaw('COUNT(*) as attempt_count')
    ->selectRaw('MAX(user_grade) as max_grade')
    ->selectRaw('(SELECT id FROM quizzes_results AS sq 
                  WHERE sq.quiz_id = quizzes_results.quiz_id 
                    AND sq.user_id = quizzes_results.user_id 
                    AND sq.user_grade = MAX(quizzes_results.user_grade) 
                  LIMIT 1) AS max_grade_result_id')
    ->groupBy('quiz_id', 'user_id')
    ->orderBy('created_at', 'desc')
    ->get();
    if ($request->has('action')) {
        ...
        if ($action === 'تصديرPDF') {
            $html = view('admin.quizzes.results_pdf', compact('quizzesResults'))->render();

            // Log the HTML for debugging
            \Log::info('Generated HTML for PDF:', ['html' => $html]);
        
            // Initialize mPDF
            $mpdf = new Mpdf([
                'mode' => 'utf-8',
                'format' => 'A4',
                'default_font' => 'XBRiyaz', // Use a font that supports RTL (e.g., DejaVu Sans)
            ...
            return $mpdf->Output('quiz_results.pdf', 'D');
        } elseif ($action === 'تصدير الى اكسل') {
            $export = new ExamsResultsExport($quizzesResults);
    ...
    public function students(Request $request, $is_export_excel = false)
    {
        ...
        $totalOrganizationsStudents = User::byBranch()->where('role_name', Role::$user)
            ->whereNotNull('organ_id')
            ->count();
        $userGroups = Group::byBranch()->where('status', 'active')
            ->orderBy('created_at', 'desc')
            ->get();

        $organizations = User::byBranch()->select('id', 'full_name', 'created_at')
    ...
    public function instructors(Request $request, $is_export_excel = false)
    {
        ...
        $query = User::byBranch()->where('role_name', Role::$teacher);
        ...
        $totalOrganizationsInstructors = User::byBranch()->where('role_name', Role::$teacher)
            ->whereNotNull('organ_id')
            ->count();
        $userGroups = Group::byBranch()->where('status', 'active')
            ->orderBy('created_at', 'desc')
    ...
    public function edit(Request $request, $id)
    {
        ...
        if ($request->get('tab') == "loginHistory") {
            ...
            $packages =\App\Models\RegistrationPackage::all();
  ...
  public function userRegistrationPackage(Request $request, $id)
{
    try {
        ...
        if (!$user->isOrganization() && !$user->isTeacher()) {
            throw new \Exception('User must be an organization or teacher');
        }
        
        if (empty($request->input('package_id'))) {
            throw new \Exception('يجب اختيار باقة');
        }

        $package = RegistrationPackage::where('id', $request->input('package_id'))
            ->where('status', 'active')
            ->firstOrFail();

        // Check if user already has this package
        $lastAccounting = $user->accounting()->latest()->first();
        if ($lastAccounting && $lastAccounting->registration_package_id == $package->id) {
            // Check if package is still valid
            $lastOrderItem = OrderItem::where('user_id', $user->id)
                ->where('registration_package_id', $package->id)
                ->latest()
                ->first();

            if ($lastOrderItem) {
                $packageStartDate = $lastOrderItem->created_at;
                $currentTime = time();
                $packageDuration = $package->days * 24 * 60 * 60; // Convert days to seconds
                
                if (($currentTime - $packageStartDate) < $packageDuration) {
                    return back()->with(['toast' => [
                        'msg' => trans('public.package_still_valid'),
                        'status' => 'error'
                    ]]);
                }
            }
        ...
        try {
            ...
            if (!empty($orderItem->registration_package_id)) {
                try {
                    Sale::create([
                        'buyer_id' => $user->id,
                        'seller_id' =>null,
                    ...
                    Accounting::createAccountingForRegistrationPackage($orderItem, 'registration_package');
                } catch (\Exception $e) {
                    DB::rollBack();
                    \Log::error('Failed to create accounting for registration package: ' . $e->getMessage());
                    throw new \Exception('Failed to process accounting records');
                }
            }
        ...
        return back()->with(['toast' => [
            'msg' => trans('public.access_denied'),
            'status' => 'error'
        ]]);
