 ...
 public function index(Request $request)
{
  ...
  $selectedCategory = null;
    ...
    // Count upcoming courses for each category and subcategory
    $categories->each(function ($category) {
        // Count upcoming courses for the parent category
        $category->upcoming_courses_count = UpcomingCourse::where('category_id', $category->id)
            ->where('status', UpcomingCourse::$active)
            ->count();

        // Count upcoming courses for each subcategory
        $category->subCategories->each(function ($subCategory) {
            $subCategory->upcoming_courses_count = UpcomingCourse::where('category_id', $subCategory->id)
                ->where('status', UpcomingCourse::$active)
                ->count();
        });

        // Calculate the total number of upcoming courses for the parent category (including subcategories)
        $category->total_upcoming_courses_count = $category->upcoming_courses_count + $category->subCategories->sum('upcoming_courses_count');

        // Remove subcategories with no upcoming courses
        $category->subCategories = $category->subCategories->filter(function ($subCategory) {
            return $subCategory->upcoming_courses_count > 0;
        });
    });

    // Remove parent categories with no upcoming courses and no subcategories with upcoming courses
    $categories = $categories->filter(function ($category) {
        return $category->total_upcoming_courses_count > 0;
    });
