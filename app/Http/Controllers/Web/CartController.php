...
class CartController extends Controller
{
    ...
    {
        $user = auth()->user();
        $carts = Cart::where('creator_id', $user->id)->where("branch_id" , 1)
            ->with([
                'user',
                'webinar',
                'event',
    ...
    {
        $user = auth()->user();
        $carts = Cart::where('creator_id', $user->id)->where("branch_id" , 3)
            ->with([
                'user',
                'webinar',
                'event',
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($calculate)) {
                ...
                return view('web.default.cart.canada_cart', $data);
            }
        } else {
            $cartDiscount = CartDiscount::query()->where('enable', true)->first();
    ...
    public function index_egy()
    {
        $user = auth()->user();
        $carts = Cart::where('creator_id', $user->id)->where("branch_id" , 4)
            ->with([
                'user',
                'webinar',
                'event',
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($calculate)) {
                ...
                return view('web.default.cart.egy_cart', $data);
            }
        } else {
            $cartDiscount = CartDiscount::query()->where('enable', true)->first();
    ...
    public function index_uae()
    {
        $user = auth()->user();
        $carts = Cart::where('creator_id', $user->id)->where("branch_id" , 2)
            ->with([
                'user',
                'webinar',
                'event',
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($calculate)) {
                ...
                return view('web.default.cart.uae_cart', $data);
            }
        } else {
            $cartDiscount = CartDiscount::query()->where('enable', true)->first();
    ...
    public function calculateProductDeliveryFee($carts)
    {
        $fee = 0;
    ...
    public function calculatePrice($carts, $user, $discountCoupon = null)
    {
        ...
        $cartHasEvent = array_filter($carts->pluck('event_id')->toArray());
        ...
        $taxIsDifferent = (count($cartHasWebinar)or count($cartHasEvent) or count($cartHasBundle) or count($cartHasMeeting) or count($cartHasInstallmentPayment));
    ...
    public function checkout_egy(Request $request, $carts = null)
    {
        $user = auth()->user();

        if (empty($carts)) {
            $carts = Cart::where('creator_id', $user->id)->where('branch_id' , 4)->get();
        }
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($order) and $order->total_amount > 0) {
                ...
                return view(getTemplate() . '.cart.payment_egy', $data);
            } else {
                return $this->handlePaymentOrderWithZeroTotalAmount($order);
            }
        }

        return redirect('/cart/egy');
    }
    public function checkout_uae(Request $request, $carts = null)
    {
        $user = auth()->user();

        if (empty($carts)) {
            $carts = Cart::where('creator_id', $user->id)->where('branch_id' , 2)->get();
        }
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($order) and $order->total_amount > 0) {
                ...
                return view(getTemplate() . '.cart.payment_uae', $data);
            } else {
                return $this->handlePaymentOrderWithZeroTotalAmount($order);
            }
        }

        return redirect('/cart/uae');
    }
    public function checkout_canada(Request $request, $carts = null)
    {
        $user = auth()->user();

        if (empty($carts)) {
            $carts = Cart::where('creator_id', $user->id)->where('branch_id', 3)->get();
        }
        ...
        if (!empty($carts) and !$carts->isEmpty()) {
            ...
            if (!empty($order) and $order->total_amount > 0) {
                ...
                return view(getTemplate() . '.cart.payment_canada', $data);
            } else {
                return $this->handlePaymentOrderWithZeroTotalAmount($order);
            }
        }
    ...
    // return $method;
    // جلب checkoutId من البوابة حسب نوع البطاقة
    if ($method === 'Visa') {
        $checkoutId = $this->generateVisaPaymentUrl($order);
    } else {
        $checkoutId = $this->generateMadaPaymentUrl($order);
    }

    // عرض صفحة الدفع وإرسال checkoutId إليها
    ...
    {
        // هنا بوابة الدفع رح ترجع بيانات الدفع
        // مثل حالة العملية، معرف الطلب، إلخ...
    ...
    private function generateMadaPaymentUrl($order)
    {
   ...
   private function generateVisaPaymentUrl($order)
{
    ...
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true); // يفضل تركه true في بيئة الإنتاج
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    if (curl_errno($ch)) {
        curl_close($ch);
        throw new \Exception("خطأ في الاتصال ببوابة الدفع: " . curl_error($ch));
    }
    curl_close($ch);
    ...
    // الحصول على checkoutId أو فشل العملية
    if (!isset($responseArray['id'])) {
        throw new \Exception("فشل في الحصول على معرف الدفع من البوابة.");
    }

    // إعادة checkoutId إلى دالة الاستدعاء
   ...
   if ($cart->webinar_id) {
    ...
    if (!empty($sessionDetails)) {
        $details = [
            'date' => $sessionDetails['date'] ?? null,
            'time' => $sessionDetails['time'] ?? null,
            'location' => $sessionDetails['location'] ?? null,
            'language' => $sessionDetails['language'] ?? null,
            'days' => $sessionDetails['days'] ?? null,
            'price' => $sessionDetails['price'] ?? null
        ];
    }
}
        ...
        if (!empty($cart->webinar_id) or !empty($cart->bundle_id)   ) {
            $item = !empty($cart->webinar_id) ? $cart->webinar : $cart->bundle;
             if (!empty($cart->webinar_id)) {
                // Get price from session if exists, otherwise use default webinar price
                $sessionDetails = session()->get('course_price_' . $cart->webinar_id);
                $price = !empty($sessionDetails['price']) ? $sessionDetails['price'] : $item->price;
            }
            else {
                // For bundles, use the default price
                $price = $item->price;
            }
