        ...
        if (empty($check) or !in_array($template, $ignoreDuplicateTemplates)) {
            ...
            if (env('APP_ENV') == 'production') {
                ...
                if (!empty($user) and !empty($user->email)) {
                    try {
                       // \Mail::to('info@growthwave-sa.com')->send(new \App\Mail\SendNotifications(['title' => $title, 'message' => $message]));
...
// To Canada
function handlePriceInUSD($price, $discount = 0, $showCurrency = true, $showTaxInPrice = true, $taxType = 'general')
{
    if (empty($price)) {
        return 0;
    }
    ...
    // 2️⃣ نحدد العملة دايمًا بالدولار
    $multiCurrency = new MultiCurrency();
    $currencies = $multiCurrency->getCurrencies();
    $usdCurrencyItem = null;
    foreach ($currencies as $currencyItem) {
        if ($currencyItem->currency == 'USD') {
            $usdCurrencyItem = $currencyItem;
            break;
        }
    }

    if (!$usdCurrencyItem) {
        $usdCurrencyItem = $multiCurrency->getDefaultCurrency(); // fallback
    }

    // 3️⃣ نحول السعر إلى الدولار
    $price = convertPriceToUserCurrency($price, $usdCurrencyItem);

    // 4️⃣ حساب الضريبة لو موجودة
    $tax = getFinancialSettings('tax') ?? 0;
    if ($taxType == 'store') {
        $storeTax = getStoreSettings('store_tax');
        if (isset($storeTax) && is_numeric($storeTax)) {
            $tax = $storeTax;
        }
    }

    $tax = convertPriceToUserCurrency($tax, $usdCurrencyItem);
    ...
    // 5️⃣ تنسيق السعر
    $decimal = $usdCurrencyItem->currency_decimal ?? 2;
    $decimalSeparator = $usdCurrencyItem->currency_separator == "dot" ? '.' : ',';
    $thousandsSeparator = $usdCurrencyItem->currency_separator == "dot" ? "," : ".";
    $price = handlePriceFormat($price, $decimal, $decimalSeparator, $thousandsSeparator);

    // 6️⃣ إضافة رمز الدولار
    if ($showCurrency) {
        $price = addCurrencyToPrice($price, $usdCurrencyItem);
    }
...
if (! function_exists('getBranchCurrency')) {
    function getBranchCurrency($branchId = 1)
    {
...
function getLeafletApiPath()
{
    ...
    //return 'https://api.mapbox.com/styles/v1/mapbox/streets-v11/tiles/{z}/{x}/{y}?access_token=env('MAPBOX_SECRET_TOKEN')';
}
...
function makeVimeoApiRequest($url, $method = 'GET', $data = [])
{
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        ...
        throw new \Exception("Vimeo API Error: " . $response);
    }

}
  function getVideosFromVimeo()
    {
       $url = 'https://api.vimeo.com/me/videos?per_page=100'; // Fetch up to 100 videos
        try {
            $response = makeVimeoApiRequest($url);
            $videos = [];

            // Loop through the videos and construct the full Vimeo URL
            foreach ($response['data'] as $video) {
                $videoId = substr($video['uri'], strrpos($video['uri'], '/') + 1); // Extract video ID
                $video['full_url'] = 'https://vimeo.com/' . $videoId; // Construct full URL
                $videos[] = $video;
            }

            return $videos; // Return the list of videos with full URLs
        } catch (\Exception $e) {
            // Handle API errors
            return [];
        }
