...
class Webinar extends Model implements TranslatableContract
{
    ...
    public $translatedAttributes = ['title', 'description', 'seo_description','sections','details','approval_name'];
   ...
   public function getApprovalNameAttribute()
    {
        return getTranslateAttributeValue($this, 'approval_name');
    }
 ...
 public function getDetailsAttribute()
  ...
  public function getSectionsAttribute()
    {
        return getTranslateAttributeValue($this, 'sections');
...
{
    return $this->hasMany(WebinarFilterOption::class, 'webinar_id');
}
public function partnerInstructors()
{
    return $this->hasMany(WebinarPartnerTeacher::class, 'webinar_id');
}
...
//branches
     public static function boot()
    {
        ...
        //     $model->branch_id = session()->get('admin_selected_branch') ?? 1;
        // });
...
public function scopeByBranch($query, $webinar = null)
{
    // If a webinar is provided and it has branches, use those branch IDs
    if ($webinar && $webinar->branches->isNotEmpty()) {
        $branchIds = $webinar->branches->pluck('id')->toArray();
        return $query->whereIn('webinars.branch_id', $branchIds);
    }

    // Fallback to session values if no webinar or branches are provided
    if (session()->has('admin_selected_branch')) {
        $branchId = session()->get('admin_selected_branch');
        return $query->where('webinars.branch_id', $branchId);
    }
    ...
    // If no conditions are met, return the query as is
    return $query;
}
...
public function getLocale()
    ...
    {
        return url('/'.$this->getLocale().'/course/' . $this->slug);
    }
