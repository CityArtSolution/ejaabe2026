...
$certificateImageUrl = url('/store/'.auth()->id().'/certificates/'.$fileName);
$params = [
    'name' => $user->full_name, // Name of the user
    'email' => $user->email, // Email of the user
    'certificate_url' =>url('/panel/quizzes/results/'.$quiz->id.'/showCertificate'),
    'certificate_name' => $quiz->webinar->title ?? '_', // Name of the certificate (same as course name)
    'certificate_image_url' => url($certificateImageUrl), // URL of the certificate image (generated dynamically)
    'browser' => $browserInfo['browser'], // Browser name
    'version' => $browserInfo['version'], // Browser version
    'course_url' => $quiz->webinar->getUrl(), // URL of the course
    'course_nameAr' => $quiz->webinar->title, // Course name in Arabic
    'instructor_name' => $quiz->webinar->teacher->full_name, // Instructor name
    'instructor_email' => $quiz->webinar->teacher->email, // Instructor email
    'platform' => 'EJAABI', // Platform name
   
]; 
$xapiService = app(XapiService::class);

$xapiService->createEarnedStatement($params);
    ...
    private function sendToApi($certificate, $html)
    {
        ...
        if (!empty($res['url'])) {
            $url = $res['url'] . ".png";
            $image =  $this->get_image_content($url);
...
$params = [
    'name' => $user->full_name, // Name of the user
    'email' => $user->email, // Email of the user
    'certificate_url' =>url('/panel/certificates/webinars/'.$certificate->id.'/show'),
    'certificate_name' => $course->title ?? '_', // Name of the certificate (same as course name)
    'certificate_image_url' => url($certificateImageUrl), // URL of the certificate image (generated dynamically)
    'browser' => $browserInfo['browser'], // Browser name
    'version' => $browserInfo['version'], // Browser version
    'course_url' => $course->getUrl(), // URL of the course
    'course_nameAr' => $course->title, // Course name in Arabic
    'instructor_name' => $course->teacher->full_name, // Instructor name
    'instructor_email' => $course->teacher->email, // Instructor email
    'platform' => 'EJAABI', // Platform name
