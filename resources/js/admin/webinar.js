    ...
    $.post(adminPanelPrefix + '/webinar-extra-description/' + item_id + '/edit', edit_data, function (result) {
        if (result && result.webinarExtraDescription) {
            ...
            Swal.fire({
                ...
                width: '48rem',
                didOpen: () => {  // Changed from onOpen to didOpen (newer version of SweetAlert2)
                    var $modal = $('#extraDescriptionModal');

                    // Set values for existing fields
                    $modal.find('[name="value"]').val(webinarExtraDescription.value);
                    $modal.find('[name="link"]').val(webinarExtraDescription.link);
                    $modal.find('[name="locale"]').val(webinarExtraDescription.locale);

                    // Handle attached file display
                    if (webinarExtraDescription.attached) {
                        const attachedContainer = $modal.find('.form-group:has([name="attached"])');
                        
                        // Remove any existing attachment display
                        attachedContainer.find('.current-attachment').remove();
                        
                        // Add the attachment display
                        const fileDisplay = `
                            <div class="mt-2 d-flex align-items-center current-attachment">
                                <a href="${adminPanelPrefix}/store/${webinarExtraDescription.attached}" class="text-primary mr-2" target="_blank">
                                    <i class="fas fa-file mr-1"></i>
                                    ${webinarExtraDescription.attached}
                                </a>
                                <a href="#" class="text-danger delete-attachment" data-id="${webinarExtraDescription.id}">
                                    <i class="fas fa-times"></i>
                                </a>
                            </div>
                        `;
                        
                        // Insert the file display after the input
                        $modal.find('[name="attached"]').after(fileDisplay);

                        // Handle delete attachment
                        $modal.find('.delete-attachment').on('click', function(e) {
                            e.preventDefault();
                            ...
                            Swal.fire({
                                ...
                                confirmButtonText: deleteConfirmButtonText, // Make sure this is defined
                                cancelButtonText: deleteCancelButtonText, // Make sure this is defined
                                icon: 'warning'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    $.post(adminPanelPrefix + '/webinar-extra-description/delete-attachment', {
                                        id: webinarExtraDescription.id,
                                        _token: $('meta[name="csrf-token"]').attr('content')
                                    }, function(response) {
                                        if (response && response.success) {
                                            $deleteBtn.closest('.current-attachment').remove();
                                            Swal.fire({
                                                text: deleteSuccessMessage, // Make sure this is defined
                                                icon: 'success'
                                            });
                                        } else {
                                            Swal.fire({
                                                text: deleteErrorMessage, // Make sure this is defined
                                                icon: 'error'
                                            });
                                        }
                                    }).fail(function() {
                    ...
                    // Make sure the form has the proper enctype for file upload
                    $modal.find('form').attr('enctype', 'multipart/form-data');
                }
            });
        }
    });
}
    function editExtraDescription111($this, locale) {
        const item_id = $this.attr('data-item-id');
        const webinar_id = $this.attr('data-webinar-id');
