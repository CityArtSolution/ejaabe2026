@php
    ...
    $locale = app()->getLocale(); // احفظ اللغة الحالية
    $rtlLanguages = !empty($generalSettings['rtl_languages']) ? $generalSettings['rtl_languages'] : [];
    $isRtl = (in_array(mb_strtoupper($locale), $rtlLanguages) || (!empty($generalSettings['rtl_layout']) && $generalSettings['rtl_layout'] == 1));
@endphp
...
@php
    $isPanel = true;
    // Check if the user is logged in and has the 'organization' role
    $isOrganization = auth()->check() && auth()->user()->role === 'organization';
@endphp

<div id="panel_app">
    <!-- Conditionally include the navbar if the user is an organization -->
    @if($isOrganization)
