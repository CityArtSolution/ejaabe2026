@php
    ...
    $user = auth()->user()->id ;
    $user_info = auth()->user() ;
    $user_scorm = ScormSession::where('user_id', $user)->get();
    $scorm_count = $user_scorm->count();
                        ...
                        <div class="col-12 ">
                            <div class="table-responsive">
                                <div class="d-flex" style="justify-content: space-between;">
                                    <div>{{ trans('update.Scorm tournament statistics') }}</div>
                                    <div style="padding: 13px;background: #faaa4b;border-radius: 31px;font-weight: bold;">{{ trans('update.Number of course visits') }} <b style="margin: 0 7px;border-bottom: 2px #136ba5 solid;"> {{ $scorm_count }} </b></div>
                                </div>
                                <table class="table custom-table text-center ">
                                    <thead>
                                    <tr>
                                        <th class="text-center text-gray">#</th>
                                        <th class="text-center text-gray">{{ trans('update.Course') }}</th>
                                        <th class="text-center text-gray">{{ trans('update.progress') }}</th>
                                        <th class="text-center text-gray">{{ trans('update.assignments') }}</th>
                                        <th class="text-center text-gray">{{ trans('update.passed_quizzes') }}</th>
                                        <th class="text-center text-gray">{{ trans('update.course_progress') }}</th>
                                        <th class="text-center text-gray">{{ trans('update.Last interaction') }}</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($user_scorm as $user_intrac)
                                        @php
                                            $data = $user_intrac->data;
                                            $lessonStatus = $data['cmi']['core']['lesson_status'] ?? null;
                                            $score = $user_intrac->score ?? null;
                                            $status = 'قيد الانتظار';
                                            if (in_array($lessonStatus, ['passed', 'completed'])) {
                                                $status = 'اجتياز';
                                            } elseif ($lessonStatus === 'failed') {
                                                $status = 'عدم اجتياز';
                                            } elseif (!$lessonStatus && $score !== null) {
                                                if ($score >= 60) {
                                                    $status = 'اجتياز';
                                                } else {
                                                    $status = 'عدم اجتياز';
                                        ...
                                        <tr>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $loop->iteration }}</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $user_intrac->webinar->title }}</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $progress }}%</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $user_intrac->score ?? 0 }}</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $status }}</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $user_intrac->completed  == 1 ? trans('update.completed') :  trans('update.not_completed') }}</span>
                                            </td>
                                            <td class="align-middle">
                                                <span class="text-dark font-weight-500">{{ $user_intrac->last_interaction_at->format('d/m/y') }}</span>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
