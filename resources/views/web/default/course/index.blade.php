...
<script>
    ...
    // إذا كان الفرع 3 => نرجع السعر مع USD فقط
    if ($course->branch_id == 3) {
        // بنشيل أي عملة من السعر ونضيف USD
        $realPriceDisplay = preg_replace('/[^\d.,]/', '', $realPrice['price']) . ' USD';
    } else {
        // الوضع العادي
        $realPriceDisplay = $realPrice['price'];
    }
@endphp
    ...
    {{ $realPriceDisplay }}
                            ...
                            <div class="mt-20 d-flex flex-column">
                                @if($course->type=='text_lesson' && !empty($course->details) && !$hasBought)
                                  ...
                                  <a href="/course/details/{{ $course->slug }}"  type="button" class="btn btn-primary">
