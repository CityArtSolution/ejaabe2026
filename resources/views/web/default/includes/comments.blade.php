<div class="mt-35">
    ...
    <div class="mt-20">
        @php
    if(auth()->check()) {
        // مستخدم مسجّل دخول => يرسل التعليق
        $commentAction = '/comments/store';
        $commentMethod = 'post';
    } else {
        // مستخدم مش مسجّل دخول => يروح على صفحة تسجيل الدخول (GET)
        $commentAction = ($course->branch_id == 3) ? '/canada/login' : '/login';
        $commentMethod = 'get';
    }
@endphp

<form action="{{ $commentAction }}" method="{{ $commentMethod }}">
