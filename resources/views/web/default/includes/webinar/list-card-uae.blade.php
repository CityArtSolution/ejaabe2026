...
<div class="webinar-card webinar-list webinar-list-2 mt-30" style="max-width: 370px;">
    ...
    <div class="webinar-card-body w-100 d-flex flex-column">
        ...
        @if(!empty($webinar->category))
            <span class="d-block font-14 mt-10" style="text-align: right;">{{ trans('public.in') }} <a href="/{{app()->getLocale()}}/uae{{ $webinar->category->getUrl() }}" target="_blank" class="text-decoration-underline">{{ $webinar->category->title }}</a></span>
        @endif
        ...
        <div class="d-flex justify-content-between mt-auto">
            ...
            <div class="webinar-price-box" style="position: absolute;background: #ffffff;padding: 7px;width: 16%;text-align: center;top: 5px;right: 6px;border-radius: 66px;">
                @if(!empty($webinar->price) and $webinar->price > 0)
                    @if($webinar->bestTicket() < $webinar->price)
                        <span class="off" style="font-size: 12px;color:green">{{ handlePrice($webinar->price, false, true, false, null, true) }} ج.م</span>
                        <span class="real" style="font-size: 12px;color:green">{{ handlePrice($webinar->bestTicket(), false, true, false, null, true) }} ج.م</span>
                    @else
                        <span class="real" style="font-size: 12px;color:green">{{ handlePrice($webinar->price, false, true, false, null, true) }} ج.م</span>
                    @endif
                @else
