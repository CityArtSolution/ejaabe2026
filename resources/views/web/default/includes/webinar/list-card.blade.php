<div class="webinar-card webinar-list webinar-list-2 d-flex mt-30">
    ...
    <div class="webinar-card-body w-100 d-flex flex-column">
        ...
        @if(!empty($webinar->category))
            <span class="d-block font-14 mt-10">{{ trans('public.in') }} <a href="{{ '/canada' . $webinar->category->getUrl() }}" target="_blank" class="text-decoration-underline">{{ $webinar->category->title }}</a></span>
        @endif
        ...
        <div class="d-flex justify-content-between mt-auto">
            ...
            <div class="webinar-price-box d-flex flex-column justify-content-center align-items-center">
                @if(!empty($webinar->price) and $webinar->price > 0)
                    @if($webinar->bestTicket() < $webinar->price)
                        <span class="off">{{ handlePrice($webinar->price, false, true, false, null, true) }} {{getBranchCurrency(3)}}</span>
                        <span class="real">{{ handlePrice($webinar->bestTicket(), false, true, false, null, true) }} {{getBranchCurrency(3)}}</span>
                    @else
