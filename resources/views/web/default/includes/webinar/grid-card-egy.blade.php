    ...
    <figure>
        ...
        <figcaption class="webinar-card-body">
           ...
           {{-- @if(!empty($webinar->category))
                <span class="d-block font-14 mt-10">{{ trans('public.in') }} <a href="/{{app()->getLocale()}}/egy{{ $webinar->category->getUrl() }}" target="_blank" class="text-decoration-underline">{{ $webinar->category->title }}</a></span>
            @endif
...
--}}
          ...
          {{--   <div class="webinar-price-box mt-25">
                ...
                @elseif(!empty($webinar->price) and $webinar->price > 0)
                    @if($webinar->bestTicket() < $webinar->price)
                        <span class="real">{{ handlePrice($webinar->bestTicket(), false, true, false, null, true) }} {{getBranchCurrency(4)}}</span>
                        <span class="off ml-10">{{ handlePrice($webinar->price, false, true, false, null, true) }} {{getBranchCurrency(4)}}</span>
                    @else
                        <span class="real">{{ handlePrice($webinar->price, false, true, false, null, true) }} {{getBranchCurrency(4)}}</span>
                    @endif
                @else
