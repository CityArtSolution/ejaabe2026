@php
    $branchCarts = $userCarts->where('branch_id', 2);
@endphp

<div class="dropdown">

    @if((empty($branchCarts) or count($branchCarts) < 1) and !empty($userCartDiscount))
        <a href="/{{app()->getLocale()}}/cart/uae/" class="btn btn-transparent">
                ...
                @if(!empty($branchCarts) and count($branchCarts) > 0)
                        ...
                        @foreach($branchCarts as $cart)
                            ...
                            @if(!empty($cartItemInfo))
                              <div class="navbar-cart-box d-flex align-items-center">
                                    <a href="{{ $cartItemInfo['itemUrl'] }}/uae" target="_blank" class="navbar-cart-img">
                                        <img src="{{ $cartItemInfo['imgPath'] }}" alt="product title" class="img-cover"/>
                                    </a>

                                    <div class="navbar-cart-info">
                                        <a href="{{ $cartItemInfo['itemUrl'] }}/uae" target="_blank">
                                            <h4>{{ $cartItemInfo['title'] }}</h4>
                                        </a>
                                        <div class="price mt-10">
                                            @if(!empty($cartItemInfo['discountPrice']))
                                                <span class="text-primary font-weight-bold">{{ handlePrice($cartItemInfo['discountPrice'], false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                                                <span class="off ml-15">{{ handlePrice($cartItemInfo['price'], false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                                            @else
                                                <span class="text-primary font-weight-bold">{{ handlePrice($cartItemInfo['price'], false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                                            @endif
                    ...
                    <div class="navbar-cart-actions">
                        <div class="navbar-cart-total mt-15 border-top d-flex align-items-center justify-content-between">
                            ...
                            <strong class="text-primary font-weight-bold">{{ !empty( $branchTotals[2]['total']) ? handlePrice( $branchTotals[2]['total'], false, true, false, null, true, $cartTaxType) : 0 }}{{getBranchCurrency(2)}}</strong>
                        </div>

                        <a href="/{{app()->getLocale()}}/cart/uae/" class="btn btn-sm btn-primary btn-block mt-50 mt-md-15">{{ trans('cart.go_to_cart') }}</a>
                    </div>
                @else
