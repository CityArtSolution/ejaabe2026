...
@php
    ...
    if(!empty($selectedCategories)) {
        // لأنك عامل radio للأب و checkbox للأبناء
        $selectedCategoryId = $selectedCategories[0] ?? null;
        $selectedCategory = $categories_filter->where('branch_id', 1)->firstWhere('id', $selectedCategoryId);

        if(!$selectedCategory) {
            // دور ضمن الأبناء كمان
...
@endphp
    ...
    <div class="container mt-30">
        <section class="mt-lg-50 pt-lg-20 mt-md-40 pt-md-40">
            <form action="/{{app()->getLocale()}}/courses" method="get" id="filtersForm">
                <div class="row mt-20">
                    <!-- Filters Column -->
            ...
            checkbox.addEventListener('click', function(event) {
                ...
                // The checkbox will now change its state
                                ...
                                <div class="mt-25 pt-25 border-top border-gray300">
                                    <h3 class="category-filter-title font-20 font-weight-bold text-dark-blue">{{ trans('public.duration_in_days') }}</h3>
                                    <div class="pt-10">
                                        @foreach($ndaysValues as $ndays)
                                        @if(!empty($ndays) ||  $ndays>0)
                                            <div class="d-flex align-items-center justify-content-between mt-20">
                                                <label class="cursor-pointer" for="filterNdays{{ $ndays }}">
                                                    {{ $ndays }} {{ $ndays ==1 ? trans('public.day') : trans('public.days') }}
                                                </label>
                                                <div class="custom-control custom-checkbox">
                                                    <input type="checkbox" name="ndays[]" id="filterNdays{{ $ndays }}" value="{{ $ndays }}" 
                                                        @if(in_array($ndays, request()->get('ndays', []))) checked="checked" @endif 
                                                        class="custom-control-input filter-checkbox">
                                                    <label class="custom-control-label" for="filterNdays{{ $ndays }}"></label>
                                                </div>
                                            </div>
                                            @endif
                                        @endforeach
