...
@section('content')
    ...
    <div class="row align-items-center">
        <div class="col-lg-4 d-flex align-items-center">
            <div class="checkbox-button primary-selected">
                <form action="{{ route('cet_courses') }}" method="get">
  ...
  <select name="ndays" id="ndaysFilter" class="form-control">
        ...
        @foreach ($uniqueNdDays as $ndays)
            <option value="{{ $ndays }}" {{ request('ndays') == $ndays ? 'selected' : '' }}>
                {{ $ndays }} 
            </option>
        @endforeach
    </select>
...
@foreach ($webinars as $loopIndex => $webinar)
    ...
    @foreach ($details as $i => $detail)
        <tr>
            {{-- العمودان الثابتان يظهران مرة واحدة مع دمج صفوف --}}
            @if ($i === 0)
    ...
    <td rowspan="{{ count($details) }}">
        <a href="/{{ app()->getLocale() }}/course/details/{{ $webinar->slug }}"
           class="btn btn-info btn-sm">{{ trans('public.details') }}</a>
    </td>
@endif
