...
@section('content')
    ...
    <div class="container">
        ...
        @if(!empty($totalCashbackAmount))
            <div class="d-flex align-items-center mt-45 p-15 success-transparent-alert">
                ...
                <div class="ml-10">
                    ...
                    <div class="font-12 ">{{ trans('update.by_purchasing_this_cart_you_will_get_amount_as_cashback',['amount' => handlePrice($totalCashbackAmount , false)]) }}{{getBranchCurrency(2)}}</div>
                </div>
            </div>
        @endif
                    ...
                    <div class="row mt-5 cart-row">
                        ...
                        if (!empty($cart->webinar_id)) {
                            ...
                            @if(!empty($cartItemInfo['discountPrice']))
                                <span class="text-gray text-decoration-line-through mx-10 mx-md-0">{{ handlePrice($cartItemInfo['price']+$price , false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cartItemInfo['discountPrice']+$price , false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                            @else
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cartItemInfo['price']+$price, false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(2)}}</span>
                            @endif
        ...
        <form action="/{{$lang}}/cart/checkout/uae" method="post" id="cartForm">
            {{ csrf_field() }}
                    ...
                    <section class="mt-45">
                        ...
                        <div class="rounded-sm shadow mt-20 pb-20 px-20">
                            ...
                            <div class="cart-checkout-item">
                                ...
                                <span class="font-14 text-gray font-weight-bold">{{ handlePrice($subTotal , false) }}{{getBranchCurrency(2)}}</span>
                            </div>
                            ...
                            <div class="cart-checkout-item">
                               ...
                               @endphp
                                 {{ number_format($discountedPrice+handlePrice($totalDiscount , false), 2) }}{{getBranchCurrency(2)}}
                                
                                @else
                                {{ handlePrice($totalDiscount , false)  }}{{getBranchCurrency(2)}}
                            ...
                            <div class="cart-checkout-item">
                                ...
                                <span class="font-14 text-gray font-weight-bold"><span id="taxPrice">{{ handlePrice($taxPrice , false) }}{{getBranchCurrency(2)}}</span></span>
                            </div>
                            ...
                            @if(!empty($productDeliveryFee))
                                <div class="cart-checkout-item">
                                    ...
                                    <span class="font-14 text-gray font-weight-bold"><span id="taxPrice">{{ handlePrice($productDeliveryFee , false) }}{{getBranchCurrency(2)}}</span></span>
                                </div>
                            @endif

                            <div class="cart-checkout-item border-0">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('cart.total') }}</h4>
                                <span class="font-14 text-gray font-weight-bold"><span id="totalAmount">{{ handlePrice($total, false) }}{{getBranchCurrency(2)}}</span></span>
                            </div>
