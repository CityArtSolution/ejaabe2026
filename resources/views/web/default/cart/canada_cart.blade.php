...
@section('content')
    <section class="cart-banner position-relative text-center">
        ...
        <span class="payment-hint font-20 text-white d-block">    {{ handlePrice($subTotal, false, true, false, null, true) . ' ' . getBranchCurrency(3) . ' ' . trans('cart.for_items',['count' => $carts->count()]) }}</span>
    </section>
    ...
    <div class="container">
        ...
        @if(!empty($totalCashbackAmount))
            <div class="d-flex align-items-center mt-45 p-15 success-transparent-alert">
                ...
                <div class="ml-10">
                    ...
                    <div class="font-12 ">{{ trans('update.by_purchasing_this_cart_you_will_get_amount_as_cashback',['amount' => handlePrice($totalCashbackAmount , false)]) }}{{getBranchCurrency(3)}}</div>
                </div>
            </div>
        @endif
                ...
                @foreach($carts as $cart)
                    <div class="row mt-5 cart-row">
                        ...
                        if (!empty($cart->webinar_id)) {
                                ...
                                $price = !empty($sessionDetails['price']) ? $sessionDetails['price'] :0;
                            } else {
                            ...
                            @if(!empty($cartItemInfo['discountPrice']))
                                <span class="text-gray text-decoration-line-through mx-10 mx-md-0">{{ handlePrice($cartItemInfo['price']+$price , false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(3)}}</span>
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cartItemInfo['discountPrice']+$price , false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(3)}}</span>
                            @else
                                <span class="font-20 text-primary mt-0 mt-md-5 font-weight-bold">{{ handlePrice($cartItemInfo['price']+$price, false, true, false, null, true, $cartTaxType) }}{{getBranchCurrency(3)}}</span>
                            @endif
        ...
        <form action="/cart/checkout/canada" method="post" id="cartForm">
            {{ csrf_field() }}
                    ...
                    <section class="mt-45">
                        ...
                        <div class="rounded-sm shadow mt-20 pb-20 px-20">
                            ...
                            <div class="cart-checkout-item">
                                ...
                                <span class="font-14 text-gray font-weight-bold">{{ handlePrice($subTotal , false) }}{{getBranchCurrency(3)}}</span>
                            </div>
                            ...
                            <div class="cart-checkout-item">
                                ...
                                @if(!empty($cart->webinar->discount_rate)&&$cart->webinar->discount_rate>0)
                                
                                 @php
                                   $sessionDetails = session()->get('course_price_' . $cart->webinar_id);
                                $price = !empty($sessionDetails['price']) ? $sessionDetails['price'] :0;
                                $discountedPrice =  $price / (1 - ( $cart->webinar->discount_rate/ 100))-$price
                               @endphp
                                 {{ number_format($discountedPrice+handlePrice($totalDiscount , false), 2) }}{{getBranchCurrency(3)}}
                                
                                @else
                                {{ handlePrice($totalDiscount , false)  }}{{getBranchCurrency(3)}}
                            ...
                            <div class="cart-checkout-item">
                                ...
                                <span class="font-14 text-gray font-weight-bold"><span id="taxPrice">{{ handlePrice($taxPrice , false) }}{{getBranchCurrency(3)}}</span></span>
                            </div>
                            ...
                            @if(!empty($productDeliveryFee))
                                <div class="cart-checkout-item">
                                    ...
                                    <span class="font-14 text-gray font-weight-bold"><span id="taxPrice">{{ handlePrice($productDeliveryFee , false) }}{{getBranchCurrency(3)}}</span></span>
                                </div>
                            @endif

                            <div class="cart-checkout-item border-0">
                                <h4 class="text-secondary font-14 font-weight-500">{{ trans('cart.total') }}</h4>
                                <span class="font-14 text-gray font-weight-bold"><span id="totalAmount">{{ handlePrice($total , false) }}{{getBranchCurrency(3)}}</span></span>
                            </div>
