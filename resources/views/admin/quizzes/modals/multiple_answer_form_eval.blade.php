<div class="add-answer-card mt-4 {{ (empty($answer) or (!empty($loop) and $loop->iteration == 1)) ? 'main-answer-row' : '' }}">
    ...
    <div class="row">
       ...
       <div class="row" style="display:none">
        <div class="col-12">
            <div class="form-group">
                <label class="input-label">افادة</label>
                <textarea type="text" name="ajax[answers][{{ !empty($answer) ? $answer->id : 'ans_tmp' }}][feedback]" class="form-control {{ !empty($answer) ? 'js-ajax-answer-feedback-'.$answer->id : '' }}" rows="1">{{ !empty($answer) ? $answer->feedback : '' }}</textarea>
            </div>
        </div>
    </div>
       ...
       @if(!empty($answer))
        <div class="col-12 col-md-4" style="display:none">
            <div class="form-group mt-2 d-flex align-items-center justify-content-between js-switch-parent">
                ...
                <div class="custom-control custom-switch">
                    <input id="correctAnswerSwitch{{ !empty($answer) ? $answer->id : '' }}" type="checkbox" name="ajax[answers][{{ !empty($answer) ? $answer->id : 'ans_tmp' }}][correct]" @if(!empty($answer) and $answer->correct) checked @endif class="custom-control-input">
        ...
        <div class="col-12 col-md-4" style="display:none">
            <div class="form-group mt-2 d-flex align-items-center justify-content-between js-switch-parent">
                <label class="js-switch" for="correctAnswerSwitch{{ !empty($answer) ? $answer->id : '' }}">{{ trans('quiz.correct_answer') }}</label>
                <div class="custom-control custom-switch">
                    <input id="correctAnswerSwitch{{ !empty($answer) ? $answer->id : '' }}" type="checkbox" name="ajax[answers][{{ !empty($answer) ? $answer->id : 'ans_tmp' }}][correct]" checked  class="custom-control-input js-switch">
                    <label class="custom-control-label js-switch" for="correctAnswerSwitch{{ !empty($answer) ? $answer->id : '' }}"></label>
                </div>
            </div>
        </div>
