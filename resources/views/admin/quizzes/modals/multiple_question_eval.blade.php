    ...
    <div class="custom-modal-body">
        ...
        <div class="quiz-questions-form" data-action="{{ getAdminPanelUrl() }}/quizzes-questions/{{ empty($question_edit) ? 'store' : $question_edit->id.'/update' }}" method="post">
            ...
            <div class="row mt-3">
                ...
                <div class="col-12">
    <div class="form-group">
        <label class="input-label" for="questionType">{{ trans('quiz.question_type') }}</label>
        <select name="ajax[type]" id="questionType" class="form-control">
            <option value="multiple" {{ !empty($question_edit) && $question_edit->type == 'multiple' ? 'selected' : '' }}>
                {{ trans('quiz.multiple_choice') }}
            </option>
            <option value="descriptive" {{ !empty($question_edit) && $question_edit->type == 'descriptive' ? 'selected' : '' }}>
        ...
        <span class="invalid-feedback"></span>
                 ...
                 <div class="col-12 col-md-12">
                    <div class="form-group">
                        <label class="input-label">اختار القسم</label>
                        <select class="form-control js-ajax-grade" name="ajax[grade]">
                        <option {{ !empty($question_edit) ? '' : 'selected' }}  disabled>اختار القسم</option>
                          @php $categories=App\Models\EvalCategory::where('quiz_id',$quiz->id)->get(); @endphp
                          @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ (((!empty($question_edit->grade) and $question_edit->grade == $category->id) or (old('category_id') == $category->id)) ? 'selected="selected"' : '') }}>{{ $category->title }}</option>
                          @endforeach
                        </select>
            ...
            <div class="add-answer-container">
                ...
                @if (!empty($question_edit->quizzesQuestionsAnswers) and !$question_edit->quizzesQuestionsAnswers->isEmpty())
                    @foreach ($question_edit->quizzesQuestionsAnswers as $answer)
                        @include('admin.quizzes.modals.multiple_answer_form_eval',['answer' => $answer])
                    @endforeach
                @else
                    @include('admin.quizzes.modals.multiple_answer_form_eval')
                @endif
            </div>
