...
@endif
            ...
            <h1>{{!empty($webinar) ?trans('/admin/main.edit'): trans('admin/main.new_eval') }} </h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="{{ getAdminPanelUrl() }}">{{ trans('admin/main.dashboard') }}</a>
                </div>
                <div class="breadcrumb-item active">
                    @if(!empty($webinar) && $webinar->type=='eval')
            
               <a href="{{ getAdminPanelUrl() }}/webinars?type=eval">{{ trans('admin/main.evals') }}</a>
                    @else
        ...
        <div class="section-body">
            ...
            <div class="row">
                <div class="col-12 ">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ getAdminPanelUrl() }}/webinars/{{ !empty($webinar) ? $webinar->id.'/update' : 'store' }}" id="webinarForm" class="webinar-form" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <input type="hidden" name="branch_ids[]" value="{{ session('admin_selected_branch') ?? 1 }}">
                                    ...
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            ...
                                            <div class="form-group mt-15">
                                                ...
                                                <select id="categories" class="custom-select @error('category_id')  is-invalid @enderror" name="category_id" required>
                                                    <option {{ !empty($webinar) ? '' : 'selected' }} disabled>{{ trans('public.choose_category') }}</option>
                                                            ...
                                                            <option value="613" >تقييمات</option>
                                            ...
                                            <div class="form-group" style="display:none">
                                                ...
                                                <input type="text" name="ajax[{{ !empty($quiz) ? $quiz->id : 'new' }}][pass_mark]" value="100" class="js-ajax-pass_mark form-control @error('pass_mark')  is-invalid @enderror" placeholder=""/>
                                                <div class="invalid-feedback"></div>
                                            </div>
                            ...
                            <section class="mt-5 col-12 ">
                                <div class="d-flex justify-content-between align-items-center pb-20">
                                    <h2 class="section-title after-line">الاقسام</h2>
                             ...
                             @endcan
                                     ...
                                     <button id="add_eval_category" data-quiz-id="{{ $quiz->id }}" type="button" class="btn btn-primary btn-sm ml-2 mt-3">إضافة الأقسام الأسئلة     </button>
                                   
                                </div>
                                @if($evalcategories)
                                    <ul class="draggable-questions-lists draggable-cat-questions-lists-{{ $quiz->id }}" data-drag-class="draggable-cat-questions-lists-{{ $quiz->id }}" data-order-table="quizzes_cat_questions" data-quiz="{{ $quiz->id }}">
                                        @foreach($evalcategories as $cat)
                                            <li data-id="{{ $cat->id }}" class="quiz-question-card d-flex align-items-center mt-4" style="width: 316px;float: right;margin: 0px 0px 0px 20px;">
                                                <div class="flex-grow-1">
                                                    <h6 class="question-title">{{ $cat->title }}</h6>
                                                </div>
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu text-left">
                                                        <button type="button" data-cat-question-id="{{ $cat->id }}" class="edit_cat_question_eval btn btn-sm btn-transparent">{{ trans('public.edit') }}</button>
                                                        @include('admin.includes.delete_button',['url' => getAdminPanelUrl('/evalcategory/'. $cat->id .'/delete'), 'btnClass' => 'btn-sm btn-transparent' , 'btnText' => trans('public.delete')])
                                                    </div>
                                                </div>
                                            </li>
                                        @endforeach
                                            ...
                                            <li data-id="{{ $question->id }}" class="quiz-question-card d-flex align-items-center mt-4">
                                                <div class="flex-grow-1">
                                                    ...
                                                    <div class="font-12 mt-3 question-infos">
                                                        <span>{{ $question->type === App\Models\QuizzesQuestion::$multiple ? trans('quiz.multiple_choice') : trans('quiz.descriptive') }}</span>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu text-left">
                                                        <button type="button" data-question-id="{{ $question->id }}" class="edit_question_eval btn btn-sm btn-transparent">{{ trans('public.edit') }}</button>
                    ...
                    @if(!empty($quiz))
                        @include('admin.quizzes.modals.multiple_question_eval')
                        @include('admin.quizzes.modals.eval_category')
                    @endif
...
@push('scripts_bottom')
    ...
    <script src="/public/assets/default/js/admin/quiz1.min.js"></script>
