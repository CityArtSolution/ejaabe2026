...
<div class="section-header">
        <h1>{{!empty($webinar) ?trans('/admin/main.edit'): trans('admin/main.new_scorm_course') }} {{ trans('admin/main.class') }}</h1>
        <div class="section-header-breadcrumb">
            ...
            <div class="breadcrumb-item">{{!empty($webinar) ?trans('/admin/main.edit'): trans('admin/main.new_scorm_course') }}</div>
        </div>
    </div>
<div class="section-body">
    <form method="post" action="{{ getAdminPanelUrl() }}/webinars/{{ !empty($webinar) ? $webinar->id.'/update' : 'store/scorm' }}" id="webinarForm" class="webinar-form" enctype="multipart/form-data" style="background:white;padding: 20px;">
        {{ csrf_field() }}
            ...
            <div class="form-group mt-15">
                <label class="form-label">{{ trans('branches.branches') }}</label>
                    <select class="form-select select2" name="branch_id" required>
                        ...
                        <option value="{{ $branch->id }}" {{ $branch->id == old('branch_id', $selectedBranchIds[0] ?? null) ? 'selected' : '' }}>
                        {{ $branch->name }}
                        </option>
                        @endforeach
                    </select>
        ...
        <label class="input-label d-block" style="background: #b8c3ed;color: #fff; padding: 5px;color: #000;font-weight: 700;font-size: 15px;">{{ trans('public.sections') }}</label> 
            ...
            <div class="col-12 col-md-5">               
                <div class="form-group mt-15">
                    <label class="input-label d-block">{{ trans('admin/main.select_a_instructor') }}</label>
                        <select name="teacher_id" data-search-option="except_user" class="form-control search-user-select22"data-placeholder="{{ trans('public.select_a_teacher') }}">
