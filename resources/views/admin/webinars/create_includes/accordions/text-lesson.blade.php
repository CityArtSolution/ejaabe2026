...
<li data-id="{{ !empty($chapterItem) ? $chapterItem->id :'' }}" class="accordion-row bg-white rounded-sm border border-gray300 mt-20 py-15 py-lg-30 px-10 px-lg-20">
    <div class="d-flex align-items-center justify-content-between " role="tab" id="text_lesson_{{ !empty($textLesson) ? $textLesson->id :'record' }}">
        <div class="d-flex align-items-center" href="#collapseTextLesson{{ !empty($textLesson) ? $textLesson->id :'record' }}" aria-controls="collapseTextLesson{{ !empty($textLesson) ? $textLesson->id :'record' }}" data-parent="#chapterContentAccordion{{ !empty($chapter) ? $chapter->id :'' }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            <div class="font-weight-bold text-dark-blue cursor-pointer" style="display:none">{{ !empty($textLesson) ? $textLesson->title : trans('public.add_new_test_lesson') }}</div>
            <div class="font-weight-bold text-dark-blue d-block cursor-pointer">{{ trans('public.textlesson') }}</div>
        </div>
        ...
        <div class="panel-collapse text-gray">
            <div class="js-content-form text_lesson-form" data-action="{{ getAdminPanelUrl() }}/text-lesson/{{ !empty($textLesson) ? $textLesson->id . '/update' : 'store' }}">
                ...
                <div class="row">
                    <div class="col-12 col-lg-6">
                       ...
                       {{-- <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ajax[{{ !empty($textLesson) ? $textLesson->id : 'new' }}][study_time]" class="js-ajax-study_time form-control" value="{{ !empty($textLesson) ? $textLesson->study_time : '0' }}" placeholder="{{ trans('forms.maximum_255_characters') }}"/>
                            <div class="invalid-feedback"></div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            <label class="input-label">{{ trans('public.Text content') }}</label>
                            <div class="content-summernote js-ajax-file_path">
                                <textarea class="js-content-summernote form-control {{ !empty($textLesson) ? 'js-content-'.$textLesson->id : '' }} custom-summernote" >{{ !empty($textLesson) ? $textLesson->content : '' }}</textarea>
