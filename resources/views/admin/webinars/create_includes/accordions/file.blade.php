                        ...
                        <div class="col-12 col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <select name="ajax[{{ !empty($file) ? $file->id : 'new' }}][storage]"
                                        ...
                                        data-part-id="{{ !empty($file) ? $file->id : 'new' }}">
                                    @foreach(getFeaturesSettings('available_sources') as $source)
                                        <option value="{{ $source }}" @if(!empty($file) and $file->storage == $source) selected @endif>
                            ...
                            <!-- Vimeo Video Selector (Initially Hidden) -->
                            <!--
<div id="vimeo-video-selector" style="display: none;">
    <select  name="vimeo_video_id" class="js-file-storage1 form-control">
        <option value="" disabled selected>اختر الفيديو المطلوب</option>
        @foreach(getVideosFromVimeo() as $video)
                                <option value="{{ $video['full_url'] }}">
                {{ $video['name'] }}
                                </option>
@endforeach
                            </select>
                            ...
                            <!-- Vimeo Video Selector Placeholder -->
                            <div id="vimeo-video-selector-{{ !empty($file) ? $file->id : 'new' }}" class="vimeo-video-selector" style="display: none;">
                                <label class="input-label">{{ trans('webinars.select_vimeo_video') }}</label>
                                <select class="js-vimeo-video form-control" name="ajax[{{ !empty($file) ? $file->id : 'new' }}][vimeo]">
                                    <option value="" disabled selected>{{ trans('webinars.select_vimeo_video_placeholder') }}</option>
                            ...
                            <div class="form-group js-file-path-input {{ (!empty($file) and $file->storage == 's3') ? 'd-none' : '' }}">
                                <div class="local-input input-group">
                                    <div class="input-group-prepend">
                                        <button type="button" class="input-group-text admin-file-manager " data-input="file_path_{{ !empty($file) ? $file->id : 'record' }}" data-preview="holder">
                                            <i data-feather="upload" width="18" height="18" class=""></i>
                                        </button>
                                    </div>
                                    <input type="text"
                                           name="ajax[{{ !empty($file) ? $file->id : 'new' }}][file_path]"
                                           id="file_path_{{ !empty($file) ? $file->id : 'record' }}"
                                           class="js-file-path form-control"
                                           value="{{ !empty($file) ? $file->file : '' }}"
                                           placeholder="{{ trans('webinars.file_upload_placeholder') }}" />                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
