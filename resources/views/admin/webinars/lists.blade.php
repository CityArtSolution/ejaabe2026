...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>  {{trans('admin/main.list')}} {{ trans('admin/main.type_'.$classesType.'s') }}</h1>
        ...
        <div class="section-body">
            ...
            <div class="row">
                ...
                @else
                    ...
                    @if($classesType !='exam' && $classesType !='eval')
                    <div class="col-lg-3 col-md-6 col-sm-6 col-12">
                        <div class="card card-statistic-1">
                            <div class="card-icon bg-info">
                                <i class="fas fa-history"></i>
                            ...
                            <div class="card-wrap">
                                ...
                                <div class="card-body">
                                    @if(in_array($classesType,['text_lesson','course']))
                                         {{$totalDurations }} {{ trans('public.days') }}
                        ...
                        <div class="card-header">
                             ...
                             @can('admin_webinars_create')
                                <div class="text-right" style="padding-left: 2rem;">
                                    <a href="{{ getAdminPanelUrl() }}/webinars/create-exam"target=_blank class="btn btn-success">{{ trans('admin/main.create_exam') }}</a>
                                </div>
                            @endcan
                            @endif
                             @if($classesType=='eval')
                             @can('admin_webinars_create')
                                <div class="text-right" style="padding-left: 2rem;">
                                    <a href="{{ getAdminPanelUrl() }}/webinars/create-eval"target=_blank class="btn btn-success">{{ trans('admin/main.create_eval') }}</a>
                                </div>
                            @endcan
                            @endif
                                    ...
                                    @foreach($webinars as $webinar)
                                        <tr class="text-center">
                                            ...
                                            <td width="18%" class="text-left">
                                                ...
                                                @else
                                  <a class="text-primary mt-0 mb-1 font-weight-bold" href="#">{{ $webinar->title }}</a>
  
                                                @endif
                                                @if(!empty($webinar->category->title))
                                                    <div class="text-small">{{ $webinar->category->title }}
                                            ...
                                            <td>
                                                ...
                                                {{ handlePrice($webinar->sales->sum('total_amount')) }}</td>
                                            @endif
                                            @if($classesType !='eval')
                                            <td class="font-12">
                                            ...
                                            @else
                                             <td class="font-12">
                                              {{   $questionCount = $webinar->quizzes->map(function($quiz) {
                                                    return $quiz->quizResults->count();
                                                })->sum()}}  
                                            ...
                                            @else
                                                <td class="font-12">{{ dateTimeFormat($webinar->updated_at, 'Y M j | H:i') }}
                                            ...
                                            <td width="200" class="">
                                                <div class="btn-group dropdown table-actions">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-left text-left webinars-lists-dropdown">
                                                        ...
                                                        @if($webinar->quizzes && $webinar->quizzes->count() > 0)
                                                        <a href="#" onclick="copyQuizLink('{{$webinar->quizzes->first()->id}}')" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1">
                                                            <i class="fa fa-link"></i>
                                                            <span class="ml-2">نسخ الرابط</span>
                                                        </a>
                                                    @else
                                                        <a href="#" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 disabled">
                                                            <i class="fa fa-bell"></i>
                                                            <span class="ml-2">لا يوجد اختبار</span>
                                                        </a>
                                                    @endif
                                                     
                                                    <script>
                                                        function copyQuizLink(quizId) {
                                                            // Get the current website URL and append the course path
                                                            const quizUrl = window.location.origin + '/panel/quizzes/' + quizId + '/start';
...
function copyQuizLink(id) {
    // بناء الرابط المطلوب نسخه
    const quizUrl = window.location.origin + '/evaluation/' + id + '/eval';

    // Clipboard API الحديثة
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(quizUrl).then(() => {
            alert('✅ تم نسخ الرابط: ' + quizUrl);
        }).catch(err => {
            alert('❌ فشل النسخ: ' + err);
        });
    } else {
        ...
        alert('✅ تم نسخ الرابط: ' + quizUrl);
                                                    ...
                                                    @else
                                                        <a href="#" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 disabled">
                                                            <i class="fa fa-bell"></i>
                                                            <span class="ml-2">  لا يوجد تقييم</span>
                                                        </a>
                                                    @endif
                                                     
                                                  
                                                         @endif
                                              @if($webinar->type!='exam' && $classesType !='eval')
                                             ...
                                             @if($classesType !='eval')
                                                ...
                                                @else
                                                @if($webinar->quizzes && $webinar->quizzes->count() > 0)
                                                 <a href="{{ getAdminPanelUrl() }}/reports/{{$webinar->quizzes->first()->id }}/eval" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="التقرير">
                                                                <i class="fas fa-file-alt"></i>
                                                                <span class="ml-2">التقرير</span>
                                                 </a>
                                                  @endif
                                                @endif
                                                    @can('admin_webinar_statistics')
                                                        @if($webinar->type=='exam' && $classesType !='eval')
                                                        @if($webinar->quizzes && $webinar->quizzes->count() > 0)
                                                            <a href="{{ getAdminPanelUrl() }}/quizzes/{{$webinar->quizzes->first()->id }}/results" target="_blank" class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-primary mt-1 " title="{{ trans('admin/main.quiz_results') }}">
                                                                <i class="fa fa-poll fa-1x"></i>
                                                                <span class="ml-2">{{ trans('admin/main.quiz_results') }}</span>
                                                            </a>
                                                             @endif
                                                              @endif
                                                        @endcan
                                          ...
                                          @if($webinar->type!='exam' && $classesType !='eval')
                                                        ...
                                                        @can('admin_webinars_edit')
                                                          ...
                                                          {{-- زر النسخ --}}
@php
   $type = $webinar->type === 'offline' ? 'webinars' : $webinar->type;
@endphp

<a href="javascript:void(0);" 
   onclick="copyQuizLinkeval('{{ $type }}', '{{ $webinar->slug }}')" 
   class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-success mt-1" 
...
{{-- سكربت النسخ --}}
<script>
function copyQuizLinkeval(type, slug) {
    // الرابط المراد نسخه
    const quizUrl = window.location.origin + '/' + 'course' + '/' + slug;

    // نسخة حديثة (Clipboard API)
    if (navigator.clipboard && window.isSecureContext) {
        navigator.clipboard.writeText(quizUrl).then(() => {
            alert('✅ تم نسخ الرابط: ' + quizUrl);
        }).catch(err => {
...
<a href="javascript:void(0);" 
   onclick="document.getElementById('duplicate-webinar-{{ $webinar->id }}').submit();" 
   class="d-flex align-items-center text-dark text-decoration-none btn-transparent btn-sm text-success mt-1" 
   title="تكرار الدورة">
    <i class="fa fa-copy"></i>
    <span class="ml-2">تكرار الدورة</span>
</a>

<form id="duplicate-webinar-{{ $webinar->id }}" 
      action="{{ route('admin.webinars.duplicate', $webinar->id) }}" 
      method="POST" 
      class="d-none">
    @csrf
</form>
