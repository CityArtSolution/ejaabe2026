                        ...
                        -->
                           ...
                           <!-- <div class="form-group custom-switches-stacked mt-2">
                                <label class="custom-switch pl-0">
                                    ...
                                    <input type="checkbox" name="ai_content_access" id="aiContentAccessSwitch" value="1" {{ (!empty($package) and $package->ai_content_access) ? 'checked="checked"' : '' }} class="custom-switch-input"/>
                                    <span class="custom-switch-indicator"></span>
                                    <label class="custom-switch-description mb-0 cursor-pointer" for="aiContentAccessSwitch">{{ trans('update.ai_content_access') }}</label>
                                </label>
                                <div class="text-muted text-small">{{ trans('update.admin_registration_packages_ai_content_access_hint') }}</div>
                            </div>-->
                            <!--<div class="form-group js-organization-inputs js-instructor-inputs {{ ((!empty($package) and in_array($package->role, ['instructors', 'organizations'])) or in_array(old('role'), ['instructors', 'organizations'])) ? '' : 'd-none' }}">
                                <label>{{ trans('admin/main.courses') }}</label>
                                <select name="webinar_ids[]" id="webinarsSelect" class="form-control select2" multiple>
                                    @foreach($webinars as $webinar)
                                        <option value="{{ $webinar->id }}" 
                                            {{ !empty($package) && is_array($package->webinar_ids) && in_array($webinar->id, $package->webinar_ids) ? 'selected' : '' }}>
                                            {{ $webinar->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                @enderror
                                <div class="text-muted text-small mt-1">عدد الدورات المختاره يجب ان تكون مساوية لعدد الدورات الخاصة بالباقة</div>
                            </div>
-->
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#webinarsSelect').select2({
                placeholder: "{{ trans('admin/main.select_webinars') }}",
                allowClear: true
            });

            // Add validation for maximum selected webinars
            $('#webinarsSelect').on('change', function() {
                const selectedCount = $(this).val() ? $(this).val().length : 0;
                const coursesCount = parseInt($('input[name="courses_count"]').val()) || 0;

                if (selectedCount > coursesCount) {
                    alert("{{ trans('update.selected_courses_exceed_package_limit') }}");
                    $(this).val(null).trigger('change');
                }
            });

            // Update validation when courses_count changes
            $('input[name="courses_count"]').on('change keyup', function() {
                const coursesCount = parseInt($(this).val()) || 0;
                const selectedCount = $('#webinarsSelect').val() ? $('#webinarsSelect').val().length : 0;

                if (selectedCount > coursesCount) {
                    $('#webinarsSelect').val(null).trigger('change');
                    alert("{{ trans('update.courses_count_reduced_below_selected_courses') }}");
                }
            });
        });
    </script>
